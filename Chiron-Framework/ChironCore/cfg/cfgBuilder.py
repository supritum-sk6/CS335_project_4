...
def buildCFG(ir, cfgName="", isSingle=False):
    ...
    # # finding leaders in the IR
    # for idx, item in enumerate(ir):
    #     # Checking if the instruction is a jump or branch
    #     if isinstance(item[0], ChironAST.ConditionCommand):
    #         # leaderIndices.discard(idx)
    #         # The next instruction should be a leader (if exists)
    #         if idx + 1 < len(ir) and idx + 1 not in leaderIndices:
    #             leaderIndices.add(idx + 1)
    #             nextInstrLeader = BasicBlock(str(idx + 1))
    #             leader2IndicesMap[nextInstrLeader] = idx + 1
    #             indices2LeadersMap[idx + 1] = nextInstrLeader

    #         # If it's a conditional jump, mark the target as a leader
    #         if isinstance(item[0], ChironAST.ConditionCommand):
    #             targetIdx = idx + item[1]
    #             if targetIdx < len(ir) and targetIdx not in leaderIndices:
    #                 leaderIndices.add(targetIdx)
    #                 targetLeader = BasicBlock(str(targetIdx))
    #                 leader2IndicesMap[targetLeader] = targetIdx
    #                 indices2LeadersMap[targetIdx] = targetLeader
    ...
    cfg = ChironCFG(startBB, endBB, cfgName)
